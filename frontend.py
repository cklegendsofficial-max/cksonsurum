...
class VideoPipelineGUI:
    ...
    def setup_styles(self):
        ...
        style.configure(
            "Channel.TFrame", background="#3c3c3c", relief="raised", borderwidth=2
        )
    ...
    def setup_gui(self):
        ...
        self.build_exe_button = tk.Button(
            control_frame,
            text="🔨 Build EXE",
    ...
    def setup_channel_progress(self):
        ...
        for channel_name in CHANNELS_CONFIG.keys():
            channel_frame = tk.Frame(
                channels_frame, bg="#3c3c3c", relief="raised", borderwidth=2
            )
            ...
            progress_bar = ttk.Progressbar(
                channel_frame, mode="determinate", length=400
            )
    ...
    def setup_menu(self):
        ...
        pipeline_menu.add_command(
            label="🚀 Start All Channels", command=self.start_pipeline
        )
        ...
        pipeline_menu.add_command(
            label="🔍 Analyze Quality", command=self.analyze_all_videos
        )
        pipeline_menu.add_command(
            label="🎬 Video Preview", command=self.show_video_preview
        )
        pipeline_menu.add_command(
            label="🔄 Regenerate Low Quality", command=self.regenerate_low_quality
        )
        ...
        tools_menu.add_command(
            label="🔨 Build Executable", command=self.build_executable
        )
        tools_menu.add_command(
            label="📊 Performance Metrics", command=self.show_performance_metrics
        )
    ...
    def initialize_pipeline(self):
        ...
        try:
            if IMPORTS_AVAILABLE:
                ...
                self.log_message(
                    "✅ Pipeline components initialized successfully", "SYSTEM"
                )
            else:
    ...
    def setup_daily_schedule(self):
        ...
        if not SCHEDULE_AVAILABLE:
            self.log_message(
                "⚠️ Schedule library not available, using manual control", "WARNING"
            )
            return
    ...
    def start_pipeline(self):
        ...
        self.queue_progress_update(
            "button_state", button_name="start", state="disabled"
        )
        self.queue_progress_update("button_state", button_name="pause", state="normal")
        self.queue_progress_update(
            "status_update",
            status_text="🟡 Pipeline Running - Processing All Channels",
            color="#FFD700",
        )
    ...
    def pause_pipeline(self):
        ...
        self.queue_progress_update(
            "button_state", button_name="pause", state="disabled"
        )
        self.queue_progress_update(
            "status_update", status_text="🟡 Pipeline Paused", color="#FF9800"
        )
    ...
    def stop_pipeline(self):
        ...
        self.queue_progress_update(
            "status_update", status_text="🔴 Pipeline Stopped", color="#f44336"
        )
    ...
    def run_pipeline(self):
        ...
        try:
            ...
            for i, channel_name in enumerate(channels):
                ...
                self.log_message(
                    f"🎬 Starting pipeline for channel: {channel_name}", "PIPELINE"
                )
                self.update_channel_progress(
                    channel_name, 0, "🔄 Processing", "Processing"
                )
                ...
                if success:
                    self.update_channel_progress(
                        channel_name, 100, "✅ Completed", "Completed"
                    )
                    self.log_message(
                        f"✅ Channel {channel_name} completed successfully", "SUCCESS"
                    )
                else:
            ...
            if self.is_running:
                ...
                self.queue_progress_update(
                    "status_update",
                    status_text="🟢 Pipeline Completed - All Channels Processed",
                    color="#4CAF50",
                )
            ...
            self.is_running = False
            self.queue_progress_update(
                "button_state", button_name="start", state="normal"
            )
            self.queue_progress_update(
                "button_state", button_name="pause", state="disabled"
            )
    ...
    def process_channel(
        self, channel_name: str, channel_index: int, total_channels: int
    ) -> bool:
        ...
        try:
            ...
            for stage_name, progress in stages:
                ...
                self.update_channel_progress(
                    channel_name, progress, f"🔄 {stage_name}", "Processing"
                )
            ...
            self.log_message(
                f"❌ Channel {channel_name} processing failed: {e}", "ERROR"
            )
            return False

    def update_channel_progress(
        self, channel_name: str, progress: int, status_text: str, status: str
    ):
    ...
    def build_executable(self):
        ...
        try:
            ...
            try:
                ...
                self.log_message(
                    "✅ PyInstaller found, building executable...", "BUILD"
                )
                ...
                if result == 0:
                    ...
                    self.log_message(
                        "📁 Check 'dist' folder for the .exe file", "BUILD"
                    )
                    ...
                    self.log_message(
                        f"❌ Build failed with exit code: {result}", "BUILD"
                    )
                    self.queue_ui_event(
                        "show_message",
                        message_type="error",
    ...
    def analyze_all_videos(self):
        ...
        try:
            ...
            self.queue_progress_update(
                "button_state", button_name="analyze", state="disabled"
            )
            ...
            if not video_files:
                ...
                self.queue_ui_event(
                    ...
                    message_type="warning",
                    title="No Videos",
                    message="No video files found for analysis",
                )
                return

            self.log_message(
                f"📊 Found {len(video_files)} videos for analysis", "ANALYSIS"
            )

            # Start analysis in separate thread
            analysis_thread = threading.Thread(
                target=self.run_video_analysis, args=(video_files,), daemon=True
            )
            ...
            self.queue_progress_update(
                "button_state", button_name="analyze", state="normal"
            )
        ...
        try:
            for video_file in video_files:
                ...
                if analysis_result["needs_regeneration"]:
                    self.log_message(
                        f"⚠️ {video_file} needs regeneration (score: {analysis_result['quality_score']:.2f})",
                        "WARNING",
                    )
                else:
                    self.log_message(
                        f"✅ {video_file} quality acceptable (score: {analysis_result['quality_score']:.2f})",
                        "ANALYSIS",
                    )
    ...
    def analyze_single_video(self, video_path: str) -> Dict[str, any]:
        ...
        try:
            if not MOVIEPY_AVAILABLE:
                self.log_message(
                    "⚠️ MoviePy not available, using fallback analysis", "WARNING"
                )
            ...
            needs_regeneration = (
                overall_score < QUALITY_STANDARDS.get("minimum_quality_score", 0.7)
                or black_frame_ratio > 0.1
            )
            ...
            return {
                ...
                "recommendations": self.generate_improvement_recommendations(
                    overall_score, black_frame_ratio
                ),
            }
    ...
    def _analyze_video_quality(self, clip) -> float:
        ...
        try:
            ...
            sample_frames = []
            frame_count = int(
                clip.fps * min(clip.duration, 10)
            )  # Sample up to 10 seconds

            for i in range(
                0, frame_count, max(1, frame_count // 20)
            ):  # Sample 20 frames
                if i < clip.duration:
            ...
            if frame_varieties:
                variety_score = min(
                    1.0, np.std(frame_varieties) / 50.0
                )  # Normalize variety
            else:
                variety_score = 0.5
    ...
    def _detect_black_frames(self, clip) -> float:
        ...
        try:
            black_frame_count = 0
            total_frames = int(
                clip.fps * min(clip.duration, 30)
            )  # Check up to 30 seconds

            for i in range(
                0, total_frames, max(1, total_frames // 50)
            ):  # Sample 50 frames
                if i < clip.duration:
                    frame = clip.get_frame(i)
            ...
            return black_frame_count / max(
                1, total_frames // max(1, total_frames // 50)
            )
    ...
    def _fallback_video_analysis(self, video_path: str) -> Dict[str, any]:
        ...
        overall_score = (
            quality_score + duration_score + visual_score + audio_score
        ) / 4

        # Determine if regeneration is needed
        needs_regeneration = overall_score < QUALITY_STANDARDS.get(
    ...
    def generate_improvement_recommendations(
        self, quality_score: float, black_frame_ratio: float = 0.0
    ) -> List[str]:
    ...
    def regenerate_low_quality(self):
        ...
        try:
            ...
            self.queue_progress_update(
                "button_state", button_name="regenerate", state="disabled"
            )
            ...
            self.log_message(
                "🔄 Starting regeneration of low quality videos using AdvancedVideoCreator...",
                "REGENERATION",
            )
            ...
            if not low_quality_videos:
                ...
                self.queue_ui_event(
                    ...
                    message_type="info",
                    title="Regeneration",
                    message="All videos meet quality standards!",
                )
                return

            # Start regeneration process
            self.log_message(
                f"🔄 Found {len(low_quality_videos)} videos needing regeneration",
                "REGENERATION",
            )

            # Start regeneration in separate thread
            regen_thread = threading.Thread(
                target=self._run_advanced_regeneration,
                args=(low_quality_videos,),
                daemon=True,
            )
            regen_thread.start()
            ...
            self.queue_progress_update(
                "button_state", button_name="regenerate", state="normal"
            )
        ...
        try:
            for video_file, analysis in low_quality_videos:
                ...
                try:
                    ...
                    if enhanced_script:
                        self.log_message(
                            f"✅ Enhanced script created for {video_file}",
                            "REGENERATION",
                        )
                        ...
                        audio_files = self.video_creator.generate_voiceover(
                            enhanced_script, audio_folder
                        )

                        if audio_files:
                            self.log_message(
                                f"✅ New voiceover generated for {video_file}",
                                "REGENERATION",
                            )

                            # Find visual assets
                            visual_files = self.video_creator.find_visual_assets(
                                enhanced_script, "general", "assets/videos/downloads"
                            )

                            if visual_files:
                                self.log_message(
                                    f"✅ Visual assets found for {video_file}",
                                    "REGENERATION",
                                )

                                # Create regenerated video
                                regenerated_path = (
                                    f"assets/videos/regenerated_{video_file}"
                                )
                                ...
                                if final_video:
                                    self.log_message(
                                        f"🎉 Video regenerated successfully: {regenerated_path}",
                                        "REGENERATION",
                                    )
                                else:
                                    self.log_message(
                                        f"❌ Video regeneration failed for {video_file}",
                                        "ERROR",
                                    )
                            else:
                                self.log_message(
                                    f"⚠️ No visual assets found for {video_file}",
                                    "WARNING",
                                )
                        else:
                            self.log_message(
                                f"❌ Voiceover generation failed for {video_file}",
                                "ERROR",
                            )
                    else:
                        self.log_message(
                            f"❌ Enhanced script creation failed for {video_file}",
                            "ERROR",
                        )

                except Exception as video_error:
                    self.log_message(
                        f"❌ Video regeneration error for {video_file}: {video_error}",
                        "ERROR",
                    )
...
Script:"""
    ...
    def show_video_preview(self):
        ...
        try:
            ...
            self.preview_info_label = tk.Label(
                info_frame,
                text="Select a video to preview and analyze",
                bg="#2b2b2b",
                fg="#ffffff",
                font=("Arial", 10),
            )
            ...
            tk.Button(
                ...
                command=lambda: self._analyze_preview_video(
                    video_listbox, canvas, preview_window
                ),
                bg="#4CAF50",
                fg="white",
                font=("Arial", 10),
            ...
            tk.Button(
                button_frame,
                text="🔄 Regenerate",
                command=lambda: self._regenerate_preview_video(
                    video_listbox, preview_window
                ),
                bg="#FF9800",
                fg="white",
                font=("Arial", 10),
    ...
    def _show_video_frame(
        self, video_path: str, canvas: tk.Canvas, frame_time: float = 0
    ):
        ...
        try:
            ...
            if not selection:
                messagebox.showwarning(
                    "No Selection", "Please select a video to analyze"
                )
                return
...
{chr(10).join(analysis['recommendations'])}"""
    ...
    def _regenerate_preview_video(self, video_listbox, preview_window):
        ...
        try:
            ...
            if not selection:
                messagebox.showwarning(
                    "No Selection", "Please select a video to regenerate"
                )
                return
            ...
            # Start regeneration
            self.log_message(
                f"🔄 Starting regeneration for {video_file}", "REGENERATION"
            )
    ...
    def _show_frame_analysis(self, video_listbox, canvas):
        ...
        try:
            ...
            if not selection:
                messagebox.showwarning(
                    "No Selection", "Please select a video to analyze frames"
                )
                return
            ...
            if not MOVIEPY_AVAILABLE:
                messagebox.showwarning(
                    "MoviePy Required", "MoviePy is required for frame analysis"
                )
                return
            ...
            tk.Label(
                frame_window,
                text=f"🎬 Frame Analysis: {video_file}",
                font=("Arial", 16, "bold"),
                bg="#2b2b2b",
                fg="#ffffff",
            ).pack(pady=10)
            ...
            tk.Label(
                control_frame, text="Frame Time (s):", bg="#2b2b2b", fg="#ffffff"
            ).pack(side="left")
        ...
        try:
            ...
            if event_type == "channel_progress":
                ...
                self._update_channel_progress_safe(
                    channel_name, progress, status_text, status
                )
    ...
    def _update_channel_progress_safe(
        self, channel_name: str, progress: int, status_text: str, status: str
    ):
...
Built with Python, Tkinter, and Ollama integration."""
        ...
        self.queue_ui_event(
            "show_message", message_type="info", title="About", message=about_text
        )
