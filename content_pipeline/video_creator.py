    ...
    for i, scene in enumerate(scenes):
        ...
        if query:
            try:
                ...
                print(
                    f"    - UYARI (Pexels): Arama başarısız. Yedek plana geçiliyor. Hata: {e}"
                )

        if not found_video_path and last_successful_video_path:
            print("    - Yedek plan: Bir önceki sahnenin videosu kullanılıyor.")
    ...
    for i, (audio_path, visual_path) in enumerate(zip(audio_files, visual_files)):
        try:
            ...
            if (
                ...
                and os.path.getsize(visual_path) > 0
            ):
                raw_clip = VideoFileClip(visual_path)
                if raw_clip.duration < audio_clip.duration:
                    background_clip = raw_clip.fx(
                        vfx.loop, duration=audio_clip.duration
                    )
                else:
                    background_clip = raw_clip.subclip(0, audio_clip.duration)
                background_clip = background_clip.resize(height=HEIGHT).crop(
                    x_center=background_clip.w / 2, width=WIDTH, height=HEIGHT
                )
            else:
                print(
                    f"  - UYARI: Sahne {i+1} için görsel bulunamadı, siyah ekran kullanılıyor."
                )
